...
--- Handle Navigation and Selection with Keyboard and Mouse
function RageUI.HandleInput(CurrentMenu)
    if CurrentMenu == nil then return end

    -- Handle keyboard navigation
    if IsControlJustPressed(0, 172) then -- Arrow Up
        CurrentMenu.Index = (CurrentMenu.Index - 1 < 1) and #CurrentMenu.Options or CurrentMenu.Index - 1
    elseif IsControlJustPressed(0, 173) then -- Arrow Down
        CurrentMenu.Index = (CurrentMenu.Index + 1 > #CurrentMenu.Options) and 1 or CurrentMenu.Index + 1
    elseif IsControlJustPressed(0, 201) then -- Enter
        if CurrentMenu.Actions and CurrentMenu.Actions[CurrentMenu.Index] then
            CurrentMenu.Actions[CurrentMenu.Index]() -- Execute the action
        end
    end
    ...
    for i, Option in ipairs(CurrentMenu.Options) do
        local Hovered = RageUI.IsMouseInBounds(
            CurrentMenu.X,
            CurrentMenu.Y + (i - 1) * 30, -- Adjust Y based on item index
            300, -- Item width
            30   -- Item height
        )
        if Hovered then
            -- Highlight hovered item
            RenderRectangle(CurrentMenu.X, CurrentMenu.Y + (i - 1) * 30, 300, 30, 255, 255, 255, 50)
            CurrentMenu.Index = i -- Update index to match hovered item

            if IsControlJustPressed(0, 24) then -- Left Mouse Button
                if CurrentMenu.Actions and CurrentMenu.Actions[i] then
                    CurrentMenu.Actions[i]() -- Execute the action
                end
            end
        end
    end
...
--- Render the Menu
function RageUI.RenderMenu(CurrentMenu)
    if CurrentMenu == nil then return end

    -- Draw menu background
    RenderRectangle(CurrentMenu.X, CurrentMenu.Y, 300, #CurrentMenu.Options * 30, 0, 0, 0, 150)

    -- Draw menu options
    for i, Option in ipairs(CurrentMenu.Options) do
        local Color = (i == CurrentMenu.Index) and { 255, 255, 255, 255 } or { 200, 200, 200, 255 }
        RenderText(Option.Text, CurrentMenu.X + 10, CurrentMenu.Y + (i - 1) * 30 + 5, 0.35, unpack(Color))
    end

    -- Handle input for the menu
    RageUI.HandleInput(CurrentMenu)
end
